...
除 M04 轻量模式外，非轻量任务默认要求独立执行实体承接，而非由单一主对话体直接吞下。
...
如项目集成的 find-skill 可补位，则将缺口转化为技能发现任务。
...
- 路由到独立执行实体时，应使用 Agent tool
- 路由到并行执行时，应在单条消息中发起多个 Agent 调用
- 路由到独立评估时，应确保评估实体与执行实体上下文隔离
- 只有轻量且无需正式委派的任务，才允许不显式调用独立执行实体

M05 不是"口头说应当委派"，而必须在运行层面形成真正的接管关系。
