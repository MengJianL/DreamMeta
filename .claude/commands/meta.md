...
- **伤疤审计 / Scar Audit**（借鉴自 Meta_Kim Scar Protocol）：核验 Agent 在核验执行产物时，同时执行伤疤审计——扫描项目 `memory/scars/` 目录，检查是否存在与当前任务相关的历史伤疤。若存在相关伤疤，将其 `prevention_rule` 纳入核验基准。若核验中发现新的系统性失败模式（非单次 bug），在核验报告中标记「⚠️ 疑似新伤疤」，由元部门在 Phase D 的 D3 进化落盘步骤中决定是否记录。
...
| **伤疤记录** | 本轮是否暴露了系统性失败？（参考 C2 伤疤审计中标记的「⚠️ 疑似新伤疤」） | `memory/scars/`（通过 M01 记录） |
